    ...
    return (
        <div className="p-4 md:p-6 space-y-6 text-light-text dark:text-dark-text">
            ...
            <div className="bg-light-card dark:bg-dark-card p-4 rounded-lg shadow-md">
                <h2 className="text-xl font-bold mb-3">Nearest Beacon</h2>
                {nearestBeacon ? (
                    ...
                    <p className="text-light-subtext dark:text-dark-subtext">No beacons detected nearby. Waiting for signal...</p>
                )}
            </div>

            {/* Last Notification */}
            <div className="bg-light-card dark:bg-dark-card p-4 rounded-lg shadow-md">
                <h2 className="text-xl font-bold mb-3">Last Notification</h2>
                {lastNotification ? (
                    <div>
                        ...
                        <p className="text-right text-xs text-light-subtext dark:text-dark-subtext mt-2">
                            Received for {lastNotification.beacon.location} at {new Date(lastNotification.timestamp).toLocaleTimeString()}
                        </p>
                    </div>
                ) : (
            ...
            {/* Recommendations & Preferences */}
             <div className="bg-light-card dark:bg-dark-card p-4 rounded-lg shadow-md">
                ...
                <p className="text-sm text-light-subtext dark:text-dark-subtext mb-4">Select your interests to get tailored notifications. Based on your interests, you might like these exhibits:</p>
                <div className="flex flex-wrap gap-2 mb-4">
                    {(['History', 'Art', 'Science'] as BeaconCategory[]).map(cat => (
                        <button 
                            key={cat}
                 ...
                 <ul className="space-y-2">
                    ...
                    )) : <li className="text-light-subtext dark:text-dark-subtext">Select an interest to see recommendations.</li>}
                </ul>
            </div>
